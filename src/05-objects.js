...
function createObjectWithKey(key) {
  const obj = {};
    obj[key] = "random"; // this allows me to input the given key to call on my random value.
  return obj;
}
...
function createObjectWithKeyValuePair(key, val) {
  const obj = {};
  obj[key] = val; // this allows me to input the given key to call on the given value.
  return obj;
}
...
function deleteKeyFromObject(object, key) {
  delete object[key]; // used delete operation to remove the key and value from the object.
  return object;
}
