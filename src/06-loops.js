...
function targetCount(array, target) {
  ...
  let targetOccurence = 0; // tally 

for(let i = 0; i < array.length; i++) { // if 0 is less than 5 we will check if the array[i] is equal to the target, if true we add one
  if(array[i] === target) 

  targetOccurence += 1; // add one each time we see the target
}
