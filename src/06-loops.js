...
function rangeEveryOther(min, max) {
  ...
  if (range === 16) {
    //All even numbers between 10-26
    for (let i = min; i <= max; i++) {
      if (i % 2 === 0) {
        resultArray.push(i);
      }
...
function lastIndexFound(array, target) {
  ...
  for (let i = 0; i < array.length; i++) {
    if (array[i] === target) {
      lastTargetOccurence.push(i);
    }
  }
  if (lastTargetOccurence.length === 0) {
    return -1;
  }
  return lastTargetOccurence[lastTargetOccurence.length - 1];
  //console.log(lastTargetOccurence);
}
