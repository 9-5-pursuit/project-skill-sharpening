...
function getFullName(person) {
  const firstName = person.names.first;
  const middleName = person.names.middle;
  const lastName = person.names.last;
...
function getFullAddress(person) {
  ...
  const unit = person.address.unit;
...
function getFlatObject(person) {
  ...
  newPersonObj["numbers"] = person.numbers;
