...
function getLastPhoneNumber(person) {
  let storedNum = person.numbers;
  let removedNum = storedNum.pop();
  return removedNum 
}
...
function getFullAddress(person) {
  let streetNum = person.address.street;
  let unitNum = person.address.unit;
   let city = person.address.city;
    let state = person.address.state;
    let zip = person.address.zip;
    return `${streetNum} ${unitNum}, ${city}, ${state}, ${zip}`
